...
export default function CoursesRoute() {
  ...
  return (
    <>
      ...
      <div className="container grid items-center gap-2 pb-4 pt-2 text-dark-ebony md:py-5">
        <h1 className="text-3xl font-bold leading-tight tracking-tighter text-ecstasy md:text-4xl">
          Courses
        </h1>
        ...
        <SearchInput
          ...
          inputPlaceholder={"Search our courses here..."}
        />
        <hr className="mb-4 w-full border-gray-300" />
        {(userDataLoading || isLoading || userAccountDataLoading) && (
          <p>Loading...</p>
        )}
        {error.length > 0 && <p>{error}</p>}
        {courses.length > 0 && (
          <h2 className="text-2xl font-bold tracking-tight">
            {(searchTerm || "Most Popular") + " Courses"}
          </h2>
        )}
          ...
          {searchTerm.length > 0 &&
            ...
            filteredCourses.map((course) => (
              <CourseCard
                key={course.courseId}
                course={course}
                userHasCourse={userHasCourse(course)}
                userHasCourseVerified={userData.verified}
              />
            ))}
          {searchTerm.length === 0 &&
            userData &&
