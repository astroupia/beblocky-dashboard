...
export default function ProgressPage({ _is_parent, credit, students }: Props) {
  ...
  return (
    <>
      ...
      <div className="mx-5 mb-5 grid items-center gap-4 lg:container">
        <TopBar name={user?.displayName as string} />
        <Tabs defaultValue="child">
          <TabsList className="mb-5 grid grid-cols-2 justify-center rounded-3xl bg-apple text-white lg:mx-[300px]">
            <TabsTrigger
              className="rounded-3xl px-10 font-bold data-[state=active]:text-apple"
              value="child"
            >
              Child
            </TabsTrigger>
          ...
          <TabsContent value="children">
            <PerformanceCards students={students} credit={credit} />
          </TabsContent>
        </Tabs>
      </div>
    </>
