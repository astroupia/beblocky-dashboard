...
export function CreateStudentDialog({ id, is_parent }: Props) {
  ...
  async function onSubmit(data: z.infer<typeof FormSchema>) {
    ...
    try {
      ...
      await addDoc(
        collection(db, "School", id, "Classes", className, "Students"),
        {
          name: data.name,
          email: data.email,
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      <AlertDialog>
        <AlertDialogTrigger asChild>
          <Link href="#">
            <Card className="border-1 flex w-1/3 flex-col items-center rounded-3xl border bg-gray-100 shadow-md">
              <CardHeader>
                <div className="mr-2 rounded-full bg-apple p-2">
        ...
        <AlertDialogContent className="flex flex-col items-center bg-apple text-white">
          <AlertDialogHeader>
            <AlertDialogTitle>Add Child</AlertDialogTitle>
            <AlertDialogDescription className="text-white">
              Does your child already have an account?
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel className="border-none bg-apple px-5 py-2 opacity-50">
              Yes
            </AlertDialogCancel>
            <DialogTrigger asChild>
              <AlertDialogAction className="bg-apple opacity-50 hover:bg-gray-50 hover:text-black">
                No
              </AlertDialogAction>
            </DialogTrigger>
          </AlertDialogFooter>
      ...
      <DialogContent className="flex flex-col bg-apple text-white sm:max-w-[625px]">
        <DialogHeader>
          ...
          >
            ...
            <DialogFooter className="flex items-center justify-between">
              <Button
                variant="link"
                type="submit"
                className="bg-apple opacity-50 hover:bg-gray-50 hover:text-black"
              >
                Create Account
              </Button>
            </DialogFooter>
